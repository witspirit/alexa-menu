...
export class AmazonLoginService {
  ...
  login(onSuccess: (User) => void, onError: (AuthError) => void) {
    ...
    amazon.Login.authorize(options, (response) => { // Arrow function is required to preserve 'this' !
      this.handleAmazonAuthorize(response, onSuccess, onError);
    });
  }
  ...
  private handleAmazonAuthorize(response, onSuccess: (User) => void, onError: (AuthError) => void) {
    console.log('handleAmazonAuthorize');

    if (response.error) {
      onError(new AuthError('authorize', response.error, response.error_description));
      return;
    }
    ...
    this.retrieveProfile(user, onSuccess, onError);
  }

  private retrieveProfile(user: User, onSuccess: (User) => void, onError: (AuthError) => void) {
    ...
    amazon.Login.retrieveProfile(user.accessToken, (response) => { // Arrow function is required to preserve 'this' !
      ...
      if (response.error) {
        onError(new AuthError('profile', response.error, response.error_description));
        return;
      }
